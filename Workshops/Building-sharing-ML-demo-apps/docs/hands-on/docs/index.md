...
This should publish your image on `https://hub.docker.com/r/<your-dockerhub-username>/<some-name>`. For example, our example app image for the flower image classification app is available on `ghcr.io/scilifelabdatacentre/gradio-flower-classification:20260209-102222`. Please note that your image should stay available even after your app is published on Serve because it will be fetched with regular intervals.

!!!warning "Note"

    Image push to the dockerhub repository might take quite a bit of time due to a multitude of factors. If this is the case for you you are welcome to move on to the next part and use the image ```ghcr.io/scilifelabdatacentre/gradio-flower-classification:20260209-102222``` to complete the next part of the tutorial.
...
In order to host an app that we just built, click the Create under the **Gradio** app option. Enter the following information in the form. Leave the fields not mentioned here with default choices; they are useful for specific cases described in the detailed user guide.

* **Subdomain:** Custom subdomain of your choice. This is the subdomain that the deployed app will be available at (e.g., a subdomain of `my-gradio-app` would mean that the app will be available at `my-gradio-app.serve.scilifelab.se`). If no subdomain name is entered, a random name will be generated by default.
* **Port:** The port that the your app runs on (in our case we chose `7860`).
* **Image:** Name of the image on DockerHub or full URL to the image on a different repository (this will be either `<your-dockerhub-username>/<some-name>:<some-tag>` if you are able to push the image to Dockerhub or `ghcr.io/scilifelabdatacentre/gradio-flower-classification:20260209-102222` if you want to use an already built image).
...
* **Reason for choosing the link only option:** If you chose Link as Permissions for your app you will also need to add the reason for choosing this option. You can simply put `Part of SBDI workshop` here.

Press the **Submit** button to create the app. It should take a few minutes for your app to start (note that it will have the status Running before it is completely loaded). Once the app is available on the subdomain you chose, anyone with the link will be able to access and interact with the app on that URL (if you chose the "Link" permission level).

## Congratulations!

Congratulations, you just built and started hosting your machine learning demo application! With this knowledge, you can now go back and try a different application or learn more about specific options when building a Docker image or launching an application on SciLifeLab Serve.
